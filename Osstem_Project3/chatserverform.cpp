    ...
    ui(new Ui::ChatServerForm), totalSize(0), byteReceived(0)
{
    ...
    if (db.open()) {
        ...
        ui->chattingTreeView->setModel(chattingModel); //ui에 표시할 채팅 중인 사용자 모델
...
/* 모든 사용자의 상태가 Off 일 때 Delgate로 배경색 설정. */
void ChatServerForm::setBackground()
{
    QModelIndexList Index = serverClientModel->match(serverClientModel->index(0, 0), Qt::EditRole,
